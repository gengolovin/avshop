...
$('.del-base').on('click', function(){
    ...
    var $this = $(this),
        ...
        src = $this.data('src');
    $.ajax({
        url: adminpath + '/product/delete-baseimg',
        data: {id: id, src: src},
        type: 'POST',
        beforeSend: function(){
    ...
    new AjaxUpload(buttonSingle, {
        ...
        onComplete: function(file, response){
            setTimeout(function(){
                ...
                $('.' + buttonSingle.data('name')).html('<img src="/images/prod/' + response.file + '" style="max-height: 150px;">');
            }, 1000);
        }
    });
}
    ...
    new AjaxUpload(buttonMulti, {
        ...
        onComplete: function (file, response) {
            setTimeout(function () {
                ...
                response = JSON.parse(response);
                $('.' + buttonMulti.data('name')).append('<img src="/images/prod/' + response.file + '" style="max-height: 150px;">');
            }, 1000);
        }
    });
}
