...
class UserController extends AppController {
    ...
    public function editAction(){
        ...
        $orders = \R::getAll("SELECT `order`.`id`, `order`.`user_id`, `order`.`status_id`, `order_status`.`status`, `order`.`date`, `order`.`update_at`, `order`.`currency`, ROUND(SUM(`order_product`.`price` * `order_product`.`qty`), 2) AS `sum` FROM `order`
  JOIN `order_product` ON `order`.`id` = `order_product`.`order_id`
  JOIN `order_status`  ON `order_status`.`id` = `order`.`status_id`
  WHERE user_id = {$user_id} GROUP BY `order`.`id` ORDER BY `order`.`status_id`, `order`.`id`");
