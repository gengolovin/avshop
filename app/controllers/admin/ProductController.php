 ...
 JOIN brand ON brand.id = product.brand_id
  ORDER BY product.title LIMIT $start, $perpage");
    ...
    public function editAction(){
        ...
        App::$app->setProperty('id_brand', $product->brand_id);
    ...
    public function deleteAction(){
        $id = $this->getRequestID();
        $status_product = \R::count('product', "id = ? AND status = '1'", [$id]);
        $count_product = \R::count('order_product', 'product_id = ? ', [$id]);
        if($count_product || $status_product){
            $_SESSION['error'] = 'Удаление невозможно, данный товар есть в заказах или его статус активный';
            redirect();
        }
